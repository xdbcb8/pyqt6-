...
# -*- encoding: utf-8 -*-
'''
@File    :   LibraryManagement.py
@Time    :   2023/11/15 16:41:02
@Author  :   yangff 
@Version :   1.0
@微信公众号:  学点编程吧
'''
...
from datamanagement import BookManagement, dataBase
...
class LibraryM(QMainWindow):
    ...
    def initUI(self):
        ...
        header_labels = ["国籍（地区）", "ISBN", "书名", "作者", "图书分类", "价格"]
        ...
        self.countryLabel = QLabel(groupbox) # 作者国籍（地区）
        ...
        formLayout.addRow("国籍（地区）", self.countryLabel)
        ...
        countryAct = QAction("设置国籍（地区）(&C)", self)
    ...
    def searchBook(self):
        ...
        if not keyword or keyword == "----": # 无搜索内容时无响应
            ...
            return
        if op == "书名":
            queryResult = self.bookdb.query_book_db(subtitle=keyword)
        elif op == "作者":
            queryResult = self.bookdb.query_book_db(author=keyword)
        elif op == "ISBN":
            queryResult = self.bookdb.query_book_db(isbn=keyword)
        # 先清空表格内容
        table_rows = self.bookTable.rowCount()
        self.removeRows(table_rows)
        # 再添加查找到的书籍信息
        self.selectTable(queryResult)
        self.booklist = queryResult
    ...
    def showtable(self):
        ...
        表格呈现
        """
        # self.booklist这个变量是从数据库中获取整个图书档案列表
    ...
    def execModify(self, row, column):
        ...
        if self.bookTable.rowCount() > 0:
            # 图书的各种信息
            tablerow = row # 记录该图书在booklist上的位置
            ...
            modifyBook = BookD(1, self.bookdb, tablerow)
    ...
    def refreshdata(self, n):
        ...
        """
        self.showtable()
        if n != -100: # n!=100，表明是修改图书数据
            self.bookTable.setCurrentCell(n, 0)
            self.bookTable.cellClicked.emit(n, 0) # 让图书更多信息也刷新一下
    ...
    def removeRows(self, rows, isdel_list = 0):
        ...
        if isdel_list != 0:
            ...
            # 倒序删除，否则可能会出错！
            for i in rows:
                isbn = self.booklist[i]['isbn']
                issucess = self.bookdb.del_book_db(isbn)
                if issucess != -1:
                    self.bookTable.removeRow(i)
                    del self.booklist[i]
                else:
                    QMessageBox.critical(None, "严重错误", "图书数据删除失败", QMessageBox.StandardButton.Cancel)
            # 在数据库中删除数据，从表格中删除第i行及其所有项目，还有self.booklist中的
        else:
